...
def fallback_diagnostics_lines(
    *,
    ...
    scenario_json = json.dumps(scenario_id or None)
    return [
        f"if [ ! -f {quote(output_dir + '/multiview_diagnostics.json')} ]; then",
        f"VIDEO_PATH=$(find {quote(output_dir + '/' + view)} -type f -name '*.mp4' | sort | tail -n 1)",
        'PRIMARY_VIDEO=null',
        'VIDEO_LIST=[]',
        'if [ -n "${VIDEO_PATH:-}" ]; then PRIMARY_VIDEO="\\"$VIDEO_PATH\\""; VIDEO_LIST="[\\"$VIDEO_PATH\\"]"; fi',
        f"cat > {quote(output_dir + '/multiview_diagnostics.json')} <<JSON",
        "{",
        f"  \"checkpoint\": {json.dumps(checkpoint)},",
        "  \"diagnosis\": {",
        "    \"evidence\": {\"video_paths\": $VIDEO_LIST},",
        ...
        f"  \"scenario_id\": {scenario_json},",
        f"  \"seed\": {int(seed)},",
        f"  \"task\": {json.dumps(task)},",
        f"  \"video_length\": {int(video_length)},",
        "  \"views\": [",
        "    {",
        "      \"done_step\": null,",
        "      \"frame_count\": null,",
        ...
        "      \"policy_id\": " + json.dumps(policy_id) + ",",
        "      \"primary_video_path\": $PRIMARY_VIDEO,",
        "      \"scenario_id\": " + scenario_json + ",",
        f"      \"seed\": {int(seed)},",
        "      \"video_dir\": " + json.dumps(output_dir + "/" + view) + ",",
        "      \"video_paths\": $VIDEO_LIST,",
        f"      \"view\": {json.dumps(view)}",
        "    }",
        "  ]",
        "}",
        "JSON",
